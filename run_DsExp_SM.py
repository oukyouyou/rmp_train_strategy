...
if __name__ == "__main__":
    ...
    for i, (patient_id, f_num) in enumerate(tqdm(pids, total=len(pids), ncols=100, position=0)):
        ...
        if args.is_hyperopt:
            ...
            # max_epochs = args.train_epochs # except XGBoostTSF and SM models
            max_epochs = 50
        ...
        current_args.batch_size = 1
        ds_exp = Exp_SM_Main(current_args)

        patient_id, rmse , relative_rmse, avg_inference_time, training_cost_time , pred_loss= ds_exp.predict(settings, flag = 'test')
