...
if __name__ == "__main__":
    ...
    args.model = 'XGBoostTSF' # 'LSTM', 'MLP', 'TransformerTSFv2', 'DLinear','XGBoostTSF'
    args.pred_len= int(18) # 6, 12, 18 ,24
    max_trials = 50 
    #args.batch_size = 32
    pids = get_pids(args)
    settings = '{}_pl{}'.format(
    ...
    for i, (patient_id, f_num) in enumerate(tqdm(pids, total=len(pids), ncols=100, position=0)):
        ...
        ps_exp = Exp_xgboost(current_args)
        if not args.is_fine_tuning:
            ...
            pre_trained_path = os.path.join(args.checkpoints, 'population-level' , settings)
            if args.model == 'XGBoostTSF':
                current_args.learning_rate = current_args.learning_rate * 0.5
                load_model_path = os.path.join(pre_trained_path + '/xgboost_model.json')
            else:
                current_args.learning_rate = current_args.learning_rate * 0.5
