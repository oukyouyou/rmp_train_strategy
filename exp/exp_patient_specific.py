...
class Exp_PS_Main(Exp_Basic):
    def __init__(self, args):
        super(Exp_PS_Main, self).__init__(args)
        self.args = args
    ...
    def train(self, setting, load_model_path=None):
        pid_settings = 'pid{}_fnum{}'.format(
                ...
                self.args.f_num)
        logger.info(f"-------------------Start training on patient-specific dataset: {pid_settings} -------------------------")
        ...
        # logger.info(f"Best vali loss {best_vali_loss} at epoch {best_epoch}")
    ...
    def test(self, setting, model=None, load_model_path = None):
        ...
        with torch.no_grad():
            for i, batch in enumerate(test_loader):
                ...
                time_stamp = time_stamp[:, -1:, -1:].view(1)
                ...
                times_by_patient.append(time_stamp)
        ...
        with open(result_path, 'a') as f:
            ...
            no_preds = np.stack(no_preds_by_patient)
            times_pred = np.stack(times_by_patient)
            ...
            np.save(os.path.join(folder_path, f"{patient_id}_time.npy"), times_pred)
            ...
            # logger.info(f"\n=== Patient {patient_id} test results ===")
            # logger.info(f'Average Inference time {avg_inference_time}')
        return patient_id, rmse , relative_rmse, avg_inference_time, model_params, macs, params 
